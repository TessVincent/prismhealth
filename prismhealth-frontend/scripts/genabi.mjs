...
const CONTRACT_NAME = "PrismHealth";

// Relative path to contract project
...
if (!fs.existsSync(dir)) {
  console.error(`${line}Unable to locate ${rel}. Expecting <root>/${dirname}${line}`);
  process.exit(1);
}
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(chainDeploymentDir) && chainId === 31337) {
    ...
    console.log("Localhost deployment not found. Please run: npx hardhat deploy --network localhost");
  }
